...
simplySlider
  ...
  to go to a specific frame:
    jQuery("#slider").simplySlider("go", position);
    "position" can have one of the following values:
    "next", "prev", "first", "last", "+1", "-1" or a numeric value
  to start/stop the automatic loop:
    jQuery("#slider").simplySlider("start");
    jQuery("#slider").simplySlider("stop");
  to change the delay between automatic transitions:
    jQuery("#slider").simplySlider("option", "delayAnimation", 1000);
  to change any option:
    jQuery("#slider").simplySlider("option", option_name, option_value);
  ...
  jQuery("#slider").data("simplySlider") will return the plugin instance and the methods/properties can be accessed from there.
  The plugin contructor defaults are accessable through SimplySlider.defaults
  The function that actually sweep the elements in/out can be overriden from
    SimplySlider.prototype._animationIn and SimplySlider.prototype._animationOut
...
;(function( $, window, document, undefined ){
  ...
  function generateNavigation($el, count, config) {
    var i, html = "",
        width = count * 24;
    html += "<ol class='" + config.navigationClass + "'>";
    for (i = 0; i < count; i++) {
  ...
  function sweepOut($el, windowWidth) {
    var dfr = jQuery.Deferred(),
        ...
        width = $el.width(),
        delta, final,
        options = $el.data("simplyAnimOptions");
    windowWidth = windowWidth || jQuery(window).width(); // check if the windowWidth is passed, if not - get it
  ...
  function sweepIn($el, windowWidth, frameLeft) {
    var dfr = jQuery.Deferred(),
        options = $el.data("simplyAnimOptions"),
        positionData = $el.data("simplyOriginalPos"),
  ...
  function absolutize($elements) {
    ...
    $elements.each(function(){
      var $t = jQuery(this);
      if ($t.data("simplyOriginalPos")) return
      $t.data("simplyOriginalPos", {
        position: $t.position(),
        width: $t.width(),
        height: $t.height(),
  ...
  var SimplySlider = function( elem, options ){
      this.elem = elem;
      this.$elem = jQuery(elem);
      this.options = options;
    };
  // the plugin prototype
  SimplySlider.prototype = {
    defaults: {
      ...
      framesSelector: ".simplyslide-cont", // selector for the frames inside the slider
      elements: {
        "p": {delay: 100, speed: 1000, easing: "easeInOutBack"}
      },
      navigation: true, // the dots navigation on the bottom
      navigationClass: "simplyslider-nav",
      // callbacks
      // another way to "catch" these events is with
      // jQuery(-slider-element-).bind("animationStart")
      animationStart: null,
      animationEnd: null
    },
    init: function() {
      var c, e, element, $element,
          that = this,
          $firstFrame;
      c = this.config = jQuery.extend({}, this.defaults, this.options);
      ...
      // bindings
      this.$elem.find(".simplyslider-nav").delegate("a", "click", function(e){
      ...
      this.$elem // internal bindings for the callbacks
        .bind("animationStart", function(){
          if (jQuery.isFunction(c.animationStart)) {c.animationStart.apply(that, arguments);}
        })
        .bind("animationEnd", function(){
          if (jQuery.isFunction(c.animationEnd)) {c.animationEnd.apply(that, arguments);}
        })
      ;
      // start animation?
      if (c.autoStart) {
    ...
    selectFrame: function(frame, dfr) {
      var c = this.config, // shorthand for the config
          that = this,
          dfr = dfr || jQuery.Deferred(),
          dFadeIn = jQuery.Deferred(),
          dFadeOut = jQuery.Deferred();
      ...
      window.setTimeout(function(){ // then wait delayBetweenTransition and fadeIn the new frame
        dFadeIn = that._fadeInFrame.call(that, frame).done(function(){
          ...
          jQuery.when(dFadeOut, dFadeIn).done(function(){
            that.animated = false;
            that.queue.shift();
    ...
    _fadeFrame: function(frame, callback, direction) {
      var dfr = jQuery.Deferred(),
          ...
          windowWidth = jQuery(window).width(), // cache it before the animations, so we don't have to check it for each element 
          i, len,
          that = this,
          elementAnimations = [],
      ...
      if (!jQuery.isFunction(callback)) return; // do nothing if there's no callback passed
      ...
      $elements.each(function(){
        elementAnimations.push( callback.call(that, jQuery(this), windowWidth, frameLeft) );
      });
      // wait for all the elements to finish their animation
      jQuery.when.apply(this, elementAnimations).done(function(){
    ...
    _getFrameElements: function(frame) { 
      var $frame = this.$frames.eq(frame),
          ...
          e, elementOptions,
          $found, $frameElements = jQuery([]);
      for (e in elements) {
        elementOptions = elements[e];
        $found = $frame.find(e);
        $found.addClass("t-frame-element").data("simplyAnimOptions", jQuery.extend({}, this.defaults.defaultElementOptions, elementOptions ));
  ...
  SimplySlider.defaults = SimplySlider.prototype.defaults;
  jQuery.fn.simplySlider = function(options) {
    var otherArgs = Array.prototype.slice.call(arguments, 1);
    return this.each(function() {
      var $el = jQuery(this),
          pluginData = $el.data("simplySlider");
      if (!pluginData) { // check if the slider is already attached
        pluginData = new SimplySlider(this, options).init();
        $el.data("simplySlider", pluginData);
        return;
      }
      ...
      if (typeof options === "string") {
        ...
        if (options === "option") {
          ...
          if (typeof otherArgs[0] === "object") { // extend the config with new options
            pluginData.config = jQuery.extend(pluginData.config, otherArgs[0]);
          }
        } else { // call a method?
          try {
  ...
  window.SimplySlider = SimplySlider;
...
jQuery(document).ready(function(){
  ...
  $slider = jQuery("#simplyslider").simplySlider({
    ...
    elements: {
      ".simplysliderimg-background": {delay: 10},
      ".simplysliderimg-background-overlay": {delay: 10},
      ".simplyslidetext-top": {delay: 500},
      "h1.simplyslider": {delay: 800},
      "img.simplyslider": {delay: 800},
      ".simplyslidetext": {delay: 1000},
      ".simplyslidedown": {delay: 1100}
    }
  });
...
jQuery(document).ready(function() {
  jQuery('.simplyslide-cont:first-child').addClass('first');
});